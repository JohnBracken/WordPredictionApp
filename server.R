...
#Back end of the Shiny word prediction app.
shinyServer(function(input, output) {

    #Once the predict button is pressed, remove all punctuation
    #from the phrase and convert the phrase to lower case letters.
    predicted_word <- eventReactive(input$predictButton, {
    test_phrase <- input$phrase
    test_phrase <- tolower(test_phrase)
    test_phrase <- gsub("[[:punct:]]", "", test_phrase) 
    
    #Use the n-gram look up tables based on what document type was selected
    #by the user using the radio buttons in the side panel on the left.  Will
    #be either blogs, news or twitter.
    if(input$doc_type=="blogs"){
    one_grams <- one_grams_blogs
    two_grams <-two_grams_blogs
    three_grams <- three_grams_blogs
    four_grams <- four_grams_blogs
    } else if(input$doc_type=="news"){
      one_grams <- one_grams_news
      two_grams <-two_grams_news
      three_grams <- three_grams_news
      four_grams <- four_grams_news
    } else{
      one_grams <- one_grams_twitter
      two_grams <-two_grams_twitter
      three_grams <- three_grams_twitter
      four_grams <- four_grams_twitter
    }
    
    
    #Make the word predcition and call the prediction function.  Return the predicted results.
    prediction <- Prediction(test_phrase, one_grams, two_grams, three_grams, four_grams)
    prediction
  })
  
  #Display the predicted words to the UI.
